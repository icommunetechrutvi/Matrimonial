...
class ProfileDetailScreen extends StatefulWidget {
  ...
  ProfileDetailScreen({Key? key, required this.profileId}) : super(key: key);
...
class _MyProfileDetailPageState extends State<ProfileDetailScreen> {
  ...
  Future<void> profileViewApi() async {
    ...
    final url = Uri.parse(
        'https://matrimonial.icommunetech.com/public/api/profile_detail/${widget.profileId}');
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        int profileHeightIndex = heightKeys.indexOf(alGetProfileDetail[0].data!.height.toString());
        ...
        int profileGenderIndex = genderKeys.indexOf(alGetProfileDetail[0].data!.gender.toString());
        if (profileGenderIndex != -1 && profileGenderIndex < genderList.length) {
          _selectedGender = genderList[profileGenderIndex];
          selectedGenderKey=alGetProfileDetail[0].data!.gender.toString();
        }

        int profileDietIndex = dietKeys.indexOf(alGetProfileDetail[0].data!.diet.toString());
        ...
        int profileBloodIndex = bloodKeys.indexOf(alGetProfileDetail[0].data!.bloodGroup.toString());
        if (profileBloodIndex != -1 && profileBloodIndex < bloodGroupList.length) {
          _selectedBlood = bloodGroupList[profileBloodIndex];
          selectedBloodKey=alGetProfileDetail[0].data!.bloodGroup.toString();
        }

        int profileMaritalIndex = maritalKeys.indexOf(alGetProfileDetail[0].data!.maritalStatus.toString());
        if (profileMaritalIndex != -1 && profileMaritalIndex < maritalStatusList.length) {
          _selectedMarital = maritalStatusList[profileMaritalIndex];
          selectedMaritalKey=alGetProfileDetail[0].data!.maritalStatus.toString();
        }

        int profileReligionIndex = religionKeys.indexOf(alGetProfileDetail[0].data!.religionId.toString());
        if (profileReligionIndex != -1 && profileReligionIndex < religionList.length) {
          _selectedReligion = religionList[profileReligionIndex];
          selectedReligionKey=alGetProfileDetail[0].data!.religionId.toString();
        }

        int profileFatherIndex = fatherKeys.indexOf(alGetProfileDetail[0].data!.fathersStatus.toString());
        if (profileFatherIndex != -1 && profileFatherIndex < fatherList.length) {
          _selectedFather = fatherList[profileFatherIndex];
          selectedFatherKey=alGetProfileDetail[0].data!.fathersStatus.toString();
        }


        int profileMotherIndex = fatherKeys.indexOf(alGetProfileDetail[0].data!.mothersStatus.toString());
        if (profileMotherIndex != -1 && profileMotherIndex < motherList.length) {
          _selectedMother = motherList[profileMotherIndex];
          selectedMotherKey=alGetProfileDetail[0].data!.mothersStatus.toString();
        }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            icon: Icon(
              Icons.favorite,
              color: wishList ? AppColor.darkGreen : AppColor.buttonColor,
            ),
          ),
          Padding(
                            ...
                            child: Card(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  _buildProfileDetailRow('Height',
                                      "${_selectedHeight}" + " "),
                                  _buildProfileDetailRow('Weight',
                                      "${profile.data!.weight}" + " Kgs"),
                                  _buildProfileDetailRow('Gender',
                                      "${_selectedGender}" + " "),
                                  _buildProfileDetailRow(
                                      'Body Type', "${selectedBodyTypeIndex != -1 ? bodyTypeList[selectedBodyTypeIndex] : null}"),
                                  _buildProfileDetailRow('Complexion',
                                      "${selectedComplexionTypeIndex != -1 ? complexionList[selectedComplexionTypeIndex] : null}"),
                                  _buildProfileDetailRow(
                                      'Diet', "${_selectedDiet}"),
                                  _buildProfileDetailRow('Blood Group',
                                      "$_selectedBlood"),
                                  _buildProfileDetailRow('Marital Status',
                                      "${_selectedMarital}"),
